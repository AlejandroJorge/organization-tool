...
<section class="flex min-h-0 flex-col space-y-4">
  <header class="flex flex-wrap items-center justify-between gap-2 text-sm text-slate-400">
    <div class="flex items-center gap-2">
      <h2 class="text-lg font-semibold text-white tracking-tight">Notes</h2>
  ...
  <form
    method="GET"
    class="rounded-2xl border border-white/10 bg-[#06080f] p-4"
  >
    <input
      type="search"
      name="q"
      ...
      class="w-full rounded-xl border border-white/15 bg-transparent px-3 py-2 text-sm text-slate-100 placeholder:text-slate-500 focus:border-white/40 focus:outline-none"
    />
  </form>

  {#if notes.length === 0}
    <div class="rounded-2xl border border-dashed border-white/10 bg-[#06080f] px-4 py-6 text-center text-sm text-slate-500">
    ...
    <div class="flex-1 min-h-0 overflow-y-auto pr-1">
      <div class="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
        {#each orderedNotes as note, index (note.id)}
          <button
            type="button"
            class={`flex min-h-[120px] flex-col gap-2 rounded-xl border border-white/10 bg-[#070a12] p-4 text-left transition hover:border-white/30 ${
          ...
          >
            <h3 class="text-sm font-semibold text-white">{note.name}</h3>
            <p class="text-xs leading-relaxed text-slate-300" class:line-clamp-5={note.content?.length && note.content.length > 280}>
              {note.content}
            </p>
          </button>
        {/each}
...
<Modal bind:isOpen={noteModalState.isOpen} onCloseRequest={handleNoteModalCloseRequest}>
  ...
  >
    ...
    <div class="space-y-1">
      <p class="text-[10px] uppercase tracking-[0.45em] text-slate-500">{noteModalState.mode} note</p>
      <h3 class="text-xl font-semibold text-white">
        {noteModalState.mode === "create" ? "New note" : "Update note"}
      </h3>
    </div>
    <div class="flex flex-col gap-2">
      <label for="name" class="text-[10px] font-semibold uppercase tracking-[0.4em] text-slate-500">Title</label>
      <input
    ...
    <div class="flex w-full flex-col gap-2">
        ...
        >
          ...
          <span class="relative inline-flex items-center">
            <input
              ...
              class="peer sr-only"
            />
            <span class="block h-4 w-8 rounded-full border border-white/15 bg-white/10 transition peer-checked:border-white/60 peer-checked:bg-white/80"></span>
      ...
      <MDInput
        ...
        class="min-h-[30vh] h-[45vh] max-h-[65vh] rounded-xl border border-white/15 bg-transparent p-3 text-sm text-white placeholder:text-slate-500 focus:border-white/40 focus:outline-none {noteModalState.isPreview ? 'overflow-y-scroll' : '' }"
      ></MDInput>
    </div>
...
<Modal bind:isOpen={noteDiscardModalOpen}>
  <div class="flex flex-col gap-4">
    <div class="space-y-1">
      ...
      <p class="text-sm text-slate-400">You have unsaved note content. Discarding will remove those edits.</p>
    </div>
    <div class="ml-auto flex gap-3">
      <button
        type="button"
